<template>
	<block>
		...
		<demo title="上下标 sup sub">
			上标<span tag="sup">0</span>
			下标<span tag="sub">0</span>
		</demo>

		<demo title="上下标 + dot">
			上标<span tag="sup dot" color="bgmain"></span>
			下标<span tag="sub dot" color="bgmain"></span>
		</demo>

		<demo>
			订单数<b tag="sup">9</b>
			消息<b tag="sup" color="bgfail white">99</b>
		...
		<demo title="tag + color">
			<span v-for="c in color.split(' ')" gap="m5" tag :color="`bg${c}-10`">
				<span :color="`${c}10`">标签</span>
			</span>
		</demo>

		<demo title="tag + color">
			<span v-for="c in color.split(' ')" gap="m5" tag :color="`bg${c}10`">
				<span :color="`${c}`">标签</span>
			</span>
		</demo>
		...
		<demo title="tag=plat + color">
			<span v-for="c in color.split(' ')" gap="m5" tag="plat" :color="`${c}10`">
				<span :color="`${c}`">标签</span>
			</span>
		</demo>
